    ...
    static func redirectNSLog(toFiles redirectToFiles: Bool, numberOfFiles: UInt = 10, sizeLimit: UInt = 0) {
        if redirectToFiles {
            ...
            for index in (0...(numberOfFiles - 2)).reversed() {
                let nsLogURLOlder: URL
                let nsLogURLCurrent: URL

                if index == 0 {
                    nsLogURLOlder = logURL(for: String("console\(subLogName).1.log"))
                    nsLogURLCurrent = logURL(for: String("console\(subLogName).log"))
                } else {
                    nsLogURLOlder = logURL(for: String("console\(subLogName).\(index + 1).log"))
                    nsLogURLCurrent = logURL(for: String("console\(subLogName).\(index).log"))
                }
                
                if fileManager.fileExists(atPath: nsLogURLCurrent.path()) {
                    if fileManager.fileExists(atPath: nsLogURLOlder.path()) {
                        // Temp log
                        log.append("removeItemAt: \(nsLogURLOlder)\n")
                        
                        do {
                            try fileManager.removeItem(at: nsLogURLOlder)
                        } catch {
                            log.append("removeItemAt: \(nsLogURLOlder). Error: \(error)\n")
                        }
                    }

                    // Temp log
                    log.append("moveItemAt: \(nsLogURLCurrent) to: \(nsLogURLOlder)\n")
                    
                    do {
                        try fileManager.moveItem(at: nsLogURLCurrent, to: nsLogURLOlder)
                    } catch {
                        log.append("moveItemAt: \(nsLogURLCurrent) to: \(nsLogURLOlder). Error: \(error)\n")
                    }
                }
            }
